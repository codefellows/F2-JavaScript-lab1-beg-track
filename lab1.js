...
var monkeysExpected = add(7, 5);

//pass when monkeysExpected is 12
assert(monkeysExpected === 12, 'seven female monkeys plus five male monkeys is twelve total.');

var monkeysExpected = add(7, 4);
//fail
assert(monkeysExpected === 12, 'seven female monkeys plus four male monkeys is eleven total, where is the other one?');
...
var splitStr = sentence1.split(' ');

for (var i = 0; i < splitStr.length; i++) {
  splitStr.pop();
  splitStr.unshift('chirp');
}

var toString = splitStr.join(' ') + '.';
...
var splitStr2 = sentence2.split(' ');

var i = 0;
do {
  i += 1;
  splitStr2.pop();
  splitStr2.unshift('chirp');
} while (i < splitStr2.length);

var toString = splitStr2.join(' ');
...
assert(animalExhibitStats.mostPopular === 'Lucky the Emperor Penguin',
  'Lucky is the most popular, as usual');
